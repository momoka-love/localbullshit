...
async function playCard(cardIndex) {
  ...
  try {
    await lobbyRef.transaction(game => {
      ...
      const serverHand = game.players[playerName] || [];
      // Remove the card safely, even if last card
      const idx = serverHand.indexOf(card);
      if (idx !== -1) serverHand.splice(idx,1);
...
function updateUI(playersObj) {
  ...
  // --- Winner detection ---
  const winner = Object.entries(playersObj).find(([_,hand]) => hand.length===0);
  ...
  playerHand.forEach((c,i)=>{
    ...
    div.style.color=(c.includes('♥')||c.includes('♦'))?'red':'black';
